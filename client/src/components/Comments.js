...
export default function Comments({ commentData, getUserName, likeData, videoId, setVideoData }) {
  ...
  return (
    <>
      <div style={{ marginBottom: "1rem", borderBottom: "1px solid gray" }}>Comments</div>
      <div className="allCommentsWrapper">
          ...
          return (
            <>
              <div style={{ display: "flex", gap: "0.25rem" }}>
                ...
                <div style={{ display: "flex", alignItems: "center", textDecoration: "underline", fontWeight: "bold" }}>{getUserName(data.userId)}</div>
              </div>
        ...
        <div className="commentsSectionBottomWrapper">
            ...
            <div style={{ display: "flex", gap: "0.25rem", marginLeft: "1rem", width: "80%" }}>
                <input type="text" placeholder="Type Comment Here..." style={{ width: "70%" }} onChange={(e)=>setComment(e.target.value)} />
                <button style={{ cursor: "pointer" }} onClick={()=>commentHandler()}>Add Comment</button>
            </div>
        </div>
    </>
  );
